...
Before posting a new version, the watcher also loads the Action1 package with `fields=versions`. Created versions include an Autodesk release-signal fingerprint in `internal_notes`. If the resolved inventory build is already recorded with the same fingerprint, the watcher adopts state without posting again. If the version exists without the matching fingerprint, the watcher fails without updating state so a stale Action1 inventory snapshot cannot hide a new Autodesk release signal.
